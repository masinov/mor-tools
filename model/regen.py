...
Exposed by _RegenMixin:
  ...
  - _deep_map_expr()
  - _recreate_variable_in_model()
  - _recreate_constraint_in_model()
  - _recreate_objective_in_model()
"""
...
class _RegenMixin:
    ...
    def clone(self) -> "EnhancedCpModel":
        ...
        # Recreate variables (with metadata)
        for name, info in self._variables.items():
            new_var = cloned._recreate_variable_in_model(info, var_mapping)
            var_mapping[name] = new_var

        # Recreate constraints (with metadata)
        for info in self._constraints.values():
            cloned._recreate_constraint_in_model(cloned, info, var_mapping)

        # Recreate objectives (with metadata)
        for obj in self._objectives:
            cloned._recreate_objective_in_model(obj, var_mapping)
    ...
    # ------------------------------------------------------------------
    # Build a clean CpModel (for I/O and debug)
    # ------------------------------------------------------------------
    def _create_solving_model(self) -> _cp.CpModel:
        ...
        for name, info in self._variables.items():
            if name.startswith("_enable_"):
                continue
            new_var = self._recreate_variable_in_model(info, var_mapping, solving_model)
            var_mapping[name] = new_var

        # Constraints
        for info in self._constraints.values():
            if info.enabled:
                self._recreate_constraint_in_model(solving_model, info, var_mapping)
    ...
    # ------------------------------------------------------------------
    # Variable recreation (with metadata)
    # ------------------------------------------------------------------
    def _recreate_variable_in_model(
        self, var_info: VariableInfo, var_mapping: Dict[str, Any], model: _cp.CpModel | None = None
    ) -> _cp.IntVar | _cp.BoolVar | _cp.IntervalVar | _cp.Constant:
        """
        Recreate a variable in the current model or a provided model using var_mapping.
        Returns the new variable and registers VariableInfo in the model.
        """
        model = model or self  # use self for clone()
        args = var_info.creation_args

        if var_info.var_type == "IntVar":
            lb, ub, _ = args
            new_var = model.NewIntVar(lb, ub, var_info.name)
        elif var_info.var_type == "BoolVar":
            new_var = model.NewBoolVar(var_info.name)
        elif var_info.var_type == "Constant":
            (value,) = args
            new_var = model.NewConstant(value)
            ...
            new_var = model.NewIntervalVar(new_start, new_size, new_end, var_info.name)
        elif var_info.var_type == "OptionalIntervalVar":
            start, size, end, is_present, _ = args
            new_start = self._map_expr_to_new_model(start, var_mapping)
            new_size = self._map_expr_to_new_model(size, var_mapping)
            new_end = self._map_expr_to_new_model(end, var_mapping)
            new_is_present = self._map_expr_to_new_model(is_present, var_mapping)
            new_var = model.NewOptionalIntervalVar(new_start, new_size, new_end, new_is_present, var_info.name)
        else:
            raise ValueError(f"Unsupported variable type: {var_info.var_type}")

        # Register VariableInfo metadata
        model._variables[var_info.name] = type(var_info)(
            name=var_info.name,
    ...
    # ------------------------------------------------------------------
    # Constraint recreation (with metadata and cloned enable_var)
    # ------------------------------------------------------------------
    def _recreate_constraint_in_model(
        self, model: _cp.CpModel, constraint_info: ConstraintInfo, var_mapping: Dict[str, Any]
    ) -> None:
        """
        Recreate a constraint in *model* using *var_mapping*.
        Replays user-enforcement literals and references cloned enable_var.
        """
        ...
        if constraint_type == "Generic":
            new_ct = model.Add(map_arg(args))
        elif constraint_type == "LinearConstraint":
            expr, lb, ub = args
            new_ct = model.AddLinearConstraint(map_arg(expr), lb, ub)
        elif constraint_type == "LinearExpressionInDomain":
            expr, dom = args
            d = _cp.Domain.FromIntervals(dom) if isinstance(dom, (list, tuple)) else dom
            new_ct = model.AddLinearExpressionInDomain(map_arg(expr), d)
        elif constraint_type == "AllDifferent":
            new_ct = model.AddAllDifferent([map_arg(v) for v in args])
        elif constraint_type == "Element":
            idx, vs, tgt = args
            new_ct = model.AddElement(map_arg(idx), [map_arg(v) for v in vs], map_arg(tgt))
        elif constraint_type == "Circuit":
            new_ct = model.AddCircuit([(h, t, map_arg(l)) for h, t, l in args])
        elif constraint_type == "MultipleCircuit":
            new_ct = model.AddMultipleCircuit([(h, t, map_arg(l)) for h, t, l in args])
        elif constraint_type == "AllowedAssignments":
            vs, ts = args
            new_ct = model.AddAllowedAssignments([map_arg(v) for v in vs], ts)
        elif constraint_type == "ForbiddenAssignments":
            vs, ts = args
            new_ct = model.AddForbiddenAssignments([map_arg(v) for v in vs], ts)
        elif constraint_type == "Automaton":
            vs, ss, fs, tr = args
            new_ct = model.AddAutomaton([map_arg(v) for v in vs], ss, list(fs), list(tr))
        elif constraint_type == "Inverse":
            vs, ivs = args
            new_ct = model.AddInverse([map_arg(v) for v in vs], [map_arg(v) for v in ivs])
        elif constraint_type == "ReservoirConstraint":
            ts, lc, mn, mx = args
            new_ct = model.AddReservoirConstraint([map_arg(t) for t in ts], [map_arg(c) for c in lc], mn, mx)
        elif constraint_type == "MinEquality":
            tgt, vs = args
            new_ct = model.AddMinEquality(map_arg(tgt), [map_arg(v) for v in vs])
        elif constraint_type == "MaxEquality":
            tgt, vs = args
            new_ct = model.AddMaxEquality(map_arg(tgt), [map_arg(v) for v in vs])
        elif constraint_type == "MultiplicationEquality":
            tgt, vs = args
            new_ct = model.AddMultiplicationEquality(map_arg(tgt), [map_arg(v) for v in vs])
        elif constraint_type == "DivisionEquality":
            tgt, n, d = args
            new_ct = model.AddDivisionEquality(map_arg(tgt), map_arg(n), map_arg(d))
        elif constraint_type == "AbsEquality":
            tgt, v = args
            new_ct = model.AddAbsEquality(map_arg(tgt), map_arg(v))
        elif constraint_type == "ModuloEquality":
            tgt, v, m = args
            new_ct = model.AddModuloEquality(map_arg(tgt), map_arg(v), map_arg(m))
        elif constraint_type == "BoolOr":
            new_ct = model.AddBoolOr([map_arg(l) for l in args])
        elif constraint_type == "BoolAnd":
            new_ct = model.AddBoolAnd([map_arg(l) for l in args])
        elif constraint_type == "BoolXor":
            new_ct = model.AddBoolXor([map_arg(l) for l in args])
        elif constraint_type == "Implication":
            a, b = args
            new_ct = model.AddImplication(map_arg(a), map_arg(b))
        elif constraint_type == "NoOverlap":
            new_ct = model.AddNoOverlap([map_arg(i) for i in args])
        elif constraint_type == "NoOverlap2D":
            xs, ys = args
            new_ct = model.AddNoOverlap2D([map_arg(i) for i in xs], [map_arg(i) for i in ys])
        elif constraint_type == "Cumulative":
            ints, dems, cap = args
            new_ct = model.AddCumulative([map_arg(i) for i in ints], [map_arg(d) for d in dems], map_arg(cap))
        else:
            raise ValueError(f"Unsupported constraint type: {constraint_type}")

        # Combine enable_var (already cloned) with user literals
        enforcement_literals = list(constraint_info.user_enforcement_literals)
        if constraint_info.enable_var is not None:
            enforcement_literals.append(var_mapping[constraint_info.enable_var.Name()])
        if enforcement_literals:
            new_ct.OnlyEnforceIf(enforcement_literals)

        # Register ConstraintInfo metadata in the cloned model
        model._constraints[constraint_info.name] = ConstraintInfo(
            name=constraint_info.name,
            original_args=args,
            constraint_type=constraint_type,
            ortools_ct=new_ct,
            enable_var=var_mapping.get(constraint_info.enable_var.Name()) if constraint_info.enable_var else None,
    ...
    # ------------------------------------------------------------------
    # Objective recreation (with metadata)
    # ------------------------------------------------------------------
    def _recreate_objective_in_model(self, obj: ObjectiveInfo, var_mapping: Dict[str, Any]) -> None:
        ...
        self._objectives.append(new_obj)

        if obj.enabled:
            if obj.objective_type == "Minimize":
                self.Minimize(new_expr)
            elif obj.objective_type == "Maximize":
                self.Maximize(new_expr)
